...
class ProfileCog(commands.Cog):
    ...
    async def profile(self, ctx, member: str = None):
        ...
        try:
            ...
            try:
                user = await self.bot.fetch_user(int(member))  # Fetch the user globally via ID or tag
                username = user.name
                ...
                await ctx.send(f"Could not find a user with that name or ID. Please ensure you've provided a valid user ID or mention.")
